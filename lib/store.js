...
function save(t) {
  cache.set(t.id, t);
  fs.writeFileSync(tournPath(t.id), JSON.stringify(t, null, 2));
  return t;
}
...
function archive(t) {
  t.status = 'finished';
  fs.writeFileSync(archivePath(t.id), JSON.stringify(t, null, 2));
...
function saveDecklist(tournamentId, userId, raw) {
  ...
  fs.writeFileSync(path.join(dir, `${userId}.txt`), raw);
}
