...
// Tournament-reminder opt-in (/notifications) + per-(tournament,user,milestone) dedup so the scheduler
// pings each lead-time only once. Stored under cfg.notify = { users:{uid:true}, sent:{key:true} }.
function setNotify(guildId, userId, on) {
  const cfg = guild(guildId); cfg.notify = cfg.notify || { users: {}, sent: {} };
  if (on) cfg.notify.users[userId] = true; else delete cfg.notify.users[userId];
  return saveGuild(guildId, cfg);
}
function getNotify(guildId, userId) { return !!(((guild(guildId).notify || {}).users) || {})[userId]; }
function notifyUsers(guildId) { return Object.keys(((guild(guildId).notify || {}).users) || {}); }
function reminderSent(guildId, key) { return !!(((guild(guildId).notify || {}).sent) || {})[key]; }
function markReminder(guildId, key) {
  const cfg = guild(guildId); cfg.notify = cfg.notify || { users: {}, sent: {} }; cfg.notify.sent = cfg.notify.sent || {};
  cfg.notify.sent[key] = true; return saveGuild(guildId, cfg);
}
...
module.exports = { guild, saveGuild, setToRole, registerChannel, unregisterChannel, channel, listChannels, setStats, setName, getName, setReadme, setGuide, setAdminGuide,
  setNotify, getNotify, notifyUsers, reminderSent, markReminder };
